...
**Navigation Conditions**:
- COADM01 → Option 1-10 → Same as COMEN01 options (All regular user functions)
- COADM01 → Additional Options → COUSR00 (User Management functions)
...
2. User enters Account Number (11 digits) → System validates format and existence
3. User presses Enter → System retrieves account data from ACCTDAT file
...
| ACRCYCR | Output | ACCTDAT file | Current cycle credit |
| ACRCYDB | Output | ACCTDAT file | Current cycle debit |
| AADDGRP | Output | ACCTDAT file | Account group ID |
...
- COACTVW → F3 → Previous Menu (COMEN01 or COADM01)
...
2. User enters Account Number → System retrieves and displays current data
...
**User Interaction Flow**:
1. User enters from menu → System displays card search screen
...
4. User selects card by entering selection number → System navigates to card details
...
| ACCTNO1-7 | Output | CARDXREF file | Account numbers for displayed cards |
| CRDNUM1-7 | Output | CARDXREF file | Card numbers for displayed cards |
| CRDSTS1-7 | Output | CARDDAT file | Card status (Active Y/N) |

**Navigation Conditions**:
- COCRDLI → Card Selection → COCRDSL (Card Detail View)
...
4. User presses Enter → System validates and updates CARDDAT file
5. User presses F3 → System returns to previous menu
...
- COCRDUP → Successful Update → COCRDUP (Confirmation message)
...
**User Interaction Flow**:
1. User enters from menu → System displays transaction search screen
...
4. User selects transaction by entering selection number → System navigates to transaction details
5. User uses F7/F8 for pagination → System displays previous/next page
6. User presses F3 → System returns to previous menu
...
5. User presses Enter with Y confirmation → System creates payment transaction and zeros balance
6. User presses F3 → System returns to previous menu
...
**User Interaction Flow**:
1. Admin user enters from admin menu → System displays user search screen
...
- COUSR02 → F3 → COADM01 (Save and exit to admin menu)
- COUSR02 → F4 → COUSR02 (Clear fields)
- COUSR02 → F5 → COUSR02 (Save changes)
- COUSR02 → F12 → COADM01 (Cancel without saving)
...
```
COSGN00 (Login)
    ├── Valid Admin User → COADM01 (Admin Menu)
    │   ├── Option 1 → COACTVW (Account View)
    │   ├── Option 2 → COACTUP (Account Update)
    │   ├── Option 3 → COCRDLI (Card List) → COCRDSL (Card Detail)
    │   ├── Option 4 → COCRDSL (Card View)
    │   ├── Option 5 → COCRDUP (Card Update)
    │   ├── Option 6 → COTRN00 (Transaction List) → COTRN01 (Transaction View)
    │   ├── Option 7 → COTRN01 (Transaction View)
    │   ├── Option 8 → COTRN02 (Transaction Add)
    │   ├── Option 9 → CORPT00 (Reports)
    │   ├── Option 10 → COBIL00 (Bill Payment)
    │   └── Admin Functions → COUSR00 (User List)
    │       ├── Add User → COUSR01
    │       ├── Update User → COUSR02
    │       └── Delete User → COUSR03
    │
    └── Valid Regular User → COMEN01 (Main Menu)
        ├── Option 1 → COACTVW (Account View)
        ├── Option 2 → COACTUP (Account Update)
        ├── Option 3 → COCRDLI (Card List) → COCRDSL (Card Detail)
        ├── Option 4 → COCRDSL (Card View)
...
- **F3 Key**: Returns to previous screen or main menu
- **Enter Key**: Processes current screen input and navigates forward
- **F4 Key**: Clears current screen fields (where applicable)

### List Screen Navigation
- **F7 Key**: Navigate to previous page
- **F8 Key**: Navigate to next page
- **Selection Fields**: Enter selection criteria to navigate to detail screens

### Data Entry Validation
- **Required Fields**: System validates all mandatory fields before processing
- **Format Validation**: Date fields, numeric fields, and code fields are validated
- **Business Rules**: Credit limits, account status, and user permissions are enforced

### Error Handling
- **Field-Level Errors**: Cursor positioned at error field with descriptive message
- **Business Rule Violations**: Clear error messages explaining the issue
- **System Errors**: Graceful handling with user-friendly error messages

This comprehensive screen flow documentation provides complete coverage of all user-facing screens in the CardDemo credit card management system, enabling users to understand the complete application workflow and navigation patterns.
