...
```mermaid
subgraph WORKFLOW_NAME["🔍 Workflow Display Name"]
...
**Source**: Extract from `screen-flow-final.md` → Summary → Main User Workflows section
...
- **Clear Visual Hierarchy**: Authentication → Menus → Workflows → Screens
...
```mermaid
flowchart TD
    subgraph AUTH["🔐 [First Workflow from Summary]"]
        SCREEN1["Screen Name<br/>PROGRAM1"]
        SCREEN2["Screen Name<br/>PROGRAM2"]
    end
    
    subgraph WORKFLOW["💳 [Second Workflow from Summary]"]
